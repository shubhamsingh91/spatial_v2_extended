...
%
    ...
    FD_qq = derivs_FD_SO.d2FD_dq;
    FD_vv = derivs_FD_SO.d2FD_dv;
    FD_qv = derivs_FD_SO.d2FD_dqv;
    FD_tau_q = derivs_FD_SO.d2FD_dtauq;
...
%       
    ...
    FD_qq_cs    = complexStepJacobian( @(x) outputSelect(1,@FD_derivatives,model,newConfig(x),qd,tau), 0*qd );
    FD_qv_cs  = complexStepJacobian( @(x) outputSelect(1,@FD_derivatives,model,q,x,qdd), qd );
    FD_vv_cs  = complexStepJacobian( @(x) outputSelect(2,@FD_derivatives,model,q,x,qdd), qd );
    FD_tau_q_cs = complexStepJacobian( @(x) outputSelect(3,@FD_derivatives,model,newConfig(x),qd,tau), 0*qd );
...
%
    checkValue('FD_qq'       , FD_qq         , FD_qq_cs                ); % SO Partials of FD w.r.t. q,q
    checkValue('FD_vv'       , FD_vv         , FD_vv_cs                ); % SO Partials of FD w.r.t. qd,qd
    checkValue('FD_qv'       , FD_qv         , FD_qv_cs                ); % SO Partials of FD w.r.t. q,qd
    checkValue('FD_tau_q'    , FD_tau_q      , FD_tau_q_cs             ); % SO Partials of FD w.r.t. tau,q
