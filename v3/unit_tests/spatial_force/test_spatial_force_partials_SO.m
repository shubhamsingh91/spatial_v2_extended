        ...
        BIic_psidk = Bten(ICi,psidk);
            ...
            if (ismember(kk,model.ancestors{jj})&&ismember(jj,model.ancestors{ii}))                                   % k<=j<=i
               ...
               %------- SO a/q Case 2C
                ...
                [d2fk_dvj_dqi_cs] = complexStepForce_SOvq(model, @(x) spatial_force_derivatives(model, newConfig(x) ,qd ,qdd, kk , jj), ...
                zeros(model.NV,1), jj, ii);
               ...
               %------- SO v/q Case 1B
                ...
                [d2fi_dvk_dqj_cs] = complexStepForce_SOvq(model, @(x) spatial_force_derivatives(model, newConfig(x) ,qd ,qdd, ii , kk), ...
                zeros(model.NV,1), kk, jj);
               ...
               [d2fi_dvj_dvk_cs] = complexStepForce_SOv(model, @(x) spatial_force_derivatives(model, q ,x ,qdd, ii , jj), ...
                ...
                d2fi_dvj_dvk = rotR(Tm( Tm(cmfM(Sj),ICi)+cmf_barM(ICi*Sj), Sk));
                  ...
                  compare('(d2fi_dvk_dvj) Case B -- algo'  , d2fi_dvj_dvk , d2fc_dv_algo{ii}(:,kk_vec,jj_vec));
