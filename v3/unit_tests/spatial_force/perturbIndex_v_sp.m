function out = perturbIndex_v_sp(model,func,x,index_jj, index_ii)
% jt_index is the joint j w.r.t which the partial is taken
  ...
  ni = model.nv(index_ii); nj = model.nv(index_jj);
     ...
     out.dfi_dqv_cs = zeros(6,nj);
     out.dfci_dqv_cs = zeros(6,nj);
    ...
    for ind=1:nj
        ...
        out.dfi_dqv_cs(:,ind) = imag(out_cs{ind}.f{index_ii})/step;
        out.dfci_dqv_cs(:,ind) = imag(out_cs{ind}.fC{index_ii})/step;
