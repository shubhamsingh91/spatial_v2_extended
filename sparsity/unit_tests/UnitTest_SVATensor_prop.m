...
M11=simplify(cmf_barM(rotR(Tm(cmfM(U),f)))-(Tm(cmfM(U),cmf_bar(f)) - mT(cmf_bar(f),crmM(U)))); % M11
...
M20 =simplify(rotT(mT(A,Y))-( Tm(rotT(Y),A.') ));                                              % M20
M21 =simplify(mT(F.',Tm(crmM(U),V))-( -rotT(mT(V.',rotR(Tm(cmf_barM(F),U)))) ) );              % M21
...
M25 =simplify( Tm(rotT(Bten(I,V)),W)   -( -rotR(Tm(rotT(Bten(I,W)),V)) ) );                    % M25
M26 =simplify( Tm(Bten(I,V),W)   -(rotR(Tm(Bten(I,W),V))-mT(I,Tm(crmM(V),W)) ) );              % M26
M27 =simplify( mT(U.',rotR(Tm(Bten(I,V),W)))   -(-rotT(mT(V.',rotR(Tm(Bten(I,U),W)))) ) );     % M27
